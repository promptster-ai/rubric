...
| **Velocity** | Whether independent work is run in parallel where it helps, and only where it helps. Within a single session: fanning independent work out to subagents, backgrounding a long-running command instead of watching it, issuing independent checks together, partitioning edits so two streams never touch the same file. Across sessions: worktrees for isolation, concurrent sessions. Both forms are the same habit at different scale; the within-session form is visible in one session's timeline, the cross-session form needs fleet correlation. | Independent work runs concurrently — subagents, backgrounded runs, isolated worktrees/sessions — partitioned so the streams land cleanly. |
